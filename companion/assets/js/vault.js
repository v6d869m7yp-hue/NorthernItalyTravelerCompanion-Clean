...
function normMatch(v){return String(v||'').toLowerCase().normalize('NFKD').replace(/[^a-z0-9]+/g,' ').trim()}
function cloudReservationPrivateMatch(cloud){
 const exact=data.records.find(r=>r.cloudReservationId===cloud.id);if(exact)return {record:exact,recovered:false};
 const name=normMatch(cloud.name),date=cloud.checkIn||cloud.eventDate||cloud.departureDate||'';
 const candidates=data.records.filter(r=>!r.cloudReservationId&&normMatch(r.title)===name);
 const dated=candidates.find(r=>(r.date||r.checkIn||'')===date);const record=dated||candidates[0]||null;
 return {record,recovered:Boolean(record)};
}
function cloudReservationPrivateRecord(cloudId){const cloud=cloudReservations.find(r=>r.id===cloudId);return cloud?cloudReservationPrivateMatch(cloud).record:data.records.find(r=>r.cloudReservationId===cloudId)}
function cloudReservationCards(){
 ...
 return `<div class="vault-record-grid cloud-reservation-grid">${cloudReservations.map(r=>{const match=cloudReservationPrivateMatch(r),privateRecord=match.record;return `<article class="card vault-record cloud-reservation-card" data-category="${esc(r.type||'hotel')}"><div class="vault-record-top"><span class="tag selected">Cloud synced</span><span class="vault-record-date">${cloudDate(r.checkIn)} → ${cloudDate(r.checkOut)}</span></div><h3>${esc(r.name)}</h3><p class="vault-location">${esc([r.city,r.country].filter(Boolean).join(', '))}</p><dl>${r.address?`<dt>Address</dt><dd>${esc(r.address)}</dd>`:''}${r.roomType?`<dt>Room</dt><dd>${esc(r.roomType)}</dd>`:''}</dl>${privateRecord?`<div class="vault-card-status">🔒 Encrypted private details attached${match.recovered?' · recovered match':''}</div>${privateRecord.attachments?.length?`<p><strong>${privateRecord.attachments.length} encrypted attachment${privateRecord.attachments.length===1?'':'s'}</strong></p>`:''}`:`<p class="notice">Add confirmation numbers, payment notes, private contacts and sensitive files here—not in the cloud reservation form.</p>`}<div class="button-row"><a class="btn outline" href="reservations.html">Cloud details</a><button class="btn" data-cloud-private="${esc(r.id)}" type="button">${privateRecord?'Edit private details':'Add private details'}</button></div></article>`}).join('')}</div>`;
}
...
async function editCloudPrivate(cloudId){
 ...
 const match=cloudReservationPrivateMatch(cloud);const existing=match.record;if(existing&&match.recovered){existing.cloudReservationId=cloudId;existing.updatedAt=now();queueChange('record-relinked',existing.id);recordAudit(`Relinked encrypted private details to ${cloud.name}`);await persist();}
...
function editDocument(id=''){
 ...
 qs('#vd-save').onclick=async()=>{const title=qs('#vd-title').value.trim();const saveButton=qs('#vd-save'),errorBox=qs('#vd-error');errorBox.textContent='';if(!title){errorBox.textContent='Enter a document title.';return}const incoming=[...qs('#vd-files').files];const removed=new Set(qsa('[data-remove-doc-file]:checked').map(x=>x.dataset.removeDocFile));saveButton.disabled=true;saveButton.textContent=incoming.length?'Preparing files…':'Saving changes…';let prepared=[];try{for(const f of incoming)prepared.push(await prepareVaultFile(f))}catch(error){errorBox.textContent=error.message||'The selected file could not be prepared.';saveButton.disabled=false;saveButton.textContent='Save encrypted document';return}const files=(d.files||[]).filter(f=>!removed.has(f.id));for(const f of prepared)files.push({id:crypto.randomUUID(),name:f.name,type:f.type,size:f.size,data:f.data});if(!files.length){errorBox.textContent='Attach at least one scan or document, or leave one existing file unchecked.';saveButton.disabled=false;saveButton.textContent='Save encrypted document';return}const out={id:id||crypto.randomUUID(),category:qs('#vd-category').value,title,holder:qs('#vd-holder').value.trim(),documentNumber:qs('#vd-number').value.trim(),issued:qs('#vd-issued').value,expires:qs('#vd-expires').value,notes:qs('#vd-notes').value.trim(),files,updatedAt:now(),updatedBy:deviceId};const documentIndex=data.documents.findIndex(x=>x.id===id);if(id&&documentIndex>=0)data.documents[documentIndex]=out;else data.documents.push(out);queueChange(id?'document-updated':'document-added',out.id);recordAudit(`${id?'Updated':'Added'} secure document: ${out.title} (${files.length} file${files.length===1?'':'s'})`);saveButton.textContent='Saving encrypted document…';try{await persist();close()}catch(error){if(id&&documentIndex>=0)data.documents[documentIndex]=d;else data.documents=data.documents.filter(x=>x.id!==out.id);errorBox.textContent=error?.name==='QuotaExceededError'?'This browser is out of secure local storage. Export a Vault backup, remove unneeded files, or use a smaller scan.':(error.message||'The encrypted document could not be saved.');saveButton.disabled=false;saveButton.textContent='Save encrypted document'}};
}
...
function fileToDataURL(file){return new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(r.result);r.onerror=()=>reject(r.error||new Error('The selected file could not be read.'));r.readAsDataURL(file)})}
function canvasToBlob(canvas,type='image/jpeg',quality=.82){return new Promise((resolve,reject)=>canvas.toBlob(blob=>blob?resolve(blob):reject(new Error('Safari could not prepare this image.')),type,quality))}
async function prepareVaultFile(file){
 const isImage=String(file.type||'').startsWith('image/');
 if(!isImage){if(file.size>5*1024*1024)throw new Error(`${file.name} is larger than 5 MB. Save large PDFs at a lower resolution before attaching.`);return {name:file.name,type:file.type||'application/octet-stream',size:file.size,data:await fileToDataURL(file)}}
 if(file.size>25*1024*1024)throw new Error(`${file.name} is larger than the 25 MB image-import limit.`);
 let bitmap;try{bitmap=await createImageBitmap(file)}catch{
  const url=URL.createObjectURL(file);try{bitmap=await new Promise((resolve,reject)=>{const img=new Image();img.onload=()=>resolve(img);img.onerror=()=>reject(new Error('Safari could not decode this image.'));img.src=url})}finally{URL.revokeObjectURL(url)}
 }
 const maxSide=1800,scale=Math.min(1,maxSide/Math.max(bitmap.width,bitmap.height));
 const canvas=document.createElement('canvas');canvas.width=Math.max(1,Math.round(bitmap.width*scale));canvas.height=Math.max(1,Math.round(bitmap.height*scale));
 const ctx=canvas.getContext('2d',{alpha:false});ctx.fillStyle='#fff';ctx.fillRect(0,0,canvas.width,canvas.height);ctx.drawImage(bitmap,0,0,canvas.width,canvas.height);if(bitmap.close)bitmap.close();
 let quality=.84,blob=await canvasToBlob(canvas,'image/jpeg',quality);
 while(blob.size>1500*1024&&quality>.55){quality-=.08;blob=await canvasToBlob(canvas,'image/jpeg',quality)}
 if(blob.size>2*1024*1024)throw new Error('This image is still too large after secure-device compression. Crop it more tightly and try again.');
 const base=(file.name||'scan').replace(/\.[^.]+$/,'');return {name:`${base}-vault.jpg`,type:'image/jpeg',size:blob.size,data:await fileToDataURL(blob)}
}
