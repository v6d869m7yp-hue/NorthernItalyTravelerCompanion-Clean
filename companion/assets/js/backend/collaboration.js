...
async function trip(){const s=requireState(),id=activeTripId();if(!id)throw new Error('Choose an active trip first.');const snap=await s.api.getDoc(s.api.doc(s.db,'trips',id));if(!snap.exists())throw new Error('The active cloud trip could not be found.');return {id:snap.id,...snap.data()};}
